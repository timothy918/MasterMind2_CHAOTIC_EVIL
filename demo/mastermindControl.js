...
function MastermindComponent(params) {
  var self = this,
    ...
    colors = ["white", "green", "black", "yellow", "red", "blue"],
    utils = {
  ...
  _.extend(self, {
    // properties and methods available to the template
    ...
    getPegClass: function (row, index) {
      ...
      if (matches && matches.hasOwnProperty("correct")) {
        return index < matches.correct
          ? "correct"
          : index < matches.correct + matches.misplaced
          ? "misplaced"
          : "";
      }
    },
    setColor: function (color) {
      // a colored marble was clicked
    ...
    submitRow: function () {
      ...
      if (
        matches.correct === params.win ||
        self.historySequence().length >= params.rows
      ) {
